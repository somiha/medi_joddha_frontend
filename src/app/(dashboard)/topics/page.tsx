"use client";

import { useEffect, useState, useCallback } from "react";
import { topicColumns } from "./columns";
import { DataTable } from "../data-table";
import { Button } from "@/components/ui/button";
import Link from "next/link";
import type { TopicWithHierarchy, TopicsApiResponse } from "./types";
import { FileText } from "lucide-react";

const BASE_URL = "https://medijoddha.save71.net";

export default function TopicsPage() {
  const [topics, setTopics] = useState<TopicWithHierarchy[]>([]);
  const [totalItems, setTotalItems] = useState<number>(0);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [pageSize, setPageSize] = useState<number>(8);

  const fetchTopics = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const token = localStorage.getItem("authToken");
      if (!token) {
        throw new Error("No authentication token found");
      }

      const url = `${BASE_URL}/api/topics?page=${currentPage}&limit=${pageSize}`;
      const res = await fetch(url, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      });

      if (!res.ok) {
        throw new Error(
          `Failed to fetch topics: ${res.status} ${res.statusText}`
        );
      }

      const data: TopicsApiResponse = await res.json();
      const topicsList = data.topics || [];

      // Enrich topics with hierarchy info
      const enrichedTopics = await Promise.all(
        topicsList.map(async (topic: TopicWithHierarchy) => {
          try {
            // Fetch chapter details
            const chapterRes = await fetch(
              `${BASE_URL}/api/chapters/${topic.chapter_id}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                  "Content-Type": "application/json",
                },
              }
            );

            if (chapterRes.ok) {
              const chapterData = await chapterRes.json();
              const chapter = chapterData.chapter;

              if (chapter) {
                topic.chapter_name = chapter.name;
                topic.chapter_title = chapter.title;

                // Fetch subject details
                const subjectRes = await fetch(
                  `${BASE_URL}/api/subjects/${chapter.subject_id}`,
                  {
                    headers: {
                      Authorization: `Bearer ${token}`,
                      "Content-Type": "application/json",
                    },
                  }
                );

                if (subjectRes.ok) {
                  const subjectData = await subjectRes.json();
                  const subject = subjectData.subject;

                  if (subject) {
                    topic.subject_name = subject.name;
                    topic.subject_title = subject.title;

                    // Fetch course details if subject has course_id
                    if (subject.course_id) {
                      const courseRes = await fetch(
                        `${BASE_URL}/api/courses/${subject.course_id}`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                            "Content-Type": "application/json",
                          },
                        }
                      );

                      if (courseRes.ok) {
                        const courseData = await courseRes.json();
                        const course = courseData.course;
                        if (course) {
                          topic.course_name = course.name;

                          // Fetch program details if course has program_id
                          if (course.program_id) {
                            const programRes = await fetch(
                              `${BASE_URL}/api/programs/${course.program_id}`,
                              {
                                headers: {
                                  Authorization: `Bearer ${token}`,
                                  "Content-Type": "application/json",
                                },
                              }
                            );

                            if (programRes.ok) {
                              const programData = await programRes.json();
                              const program = programData.program;
                              if (program) {
                                topic.program_name = program.name;
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          } catch (err) {
            console.error("Error fetching hierarchy:", err);
          }

          return topic;
        })
      );

      setTopics(enrichedTopics);
      setTotalItems(data.pagination?.totalItems || 0);
    } catch (err) {
      console.error("Error fetching topics:", err);
      setError(err instanceof Error ? err.message : "Failed to load topics");
    } finally {
      setLoading(false);
    }
  }, [currentPage, pageSize]);

  useEffect(() => {
    fetchTopics();
  }, [fetchTopics]);

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize);
    setCurrentPage(1);
  };

  const handlePageChange = (newIndex: number) => {
    setCurrentPage(newIndex + 1);
  };

  return (
    <div className="flex flex-col min-h-screen w-full p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <FileText className="w-8 h-8 text-gray-700" />
          <div>
            <h1 className="text-2xl font-bold">Topics</h1>
            <p className="text-sm text-gray-600">Manage all topics</p>
          </div>
        </div>
        <Link href="/topics/add-topic">
          <Button className="text-white hover:opacity-90">+ Add Topic</Button>
        </Link>
      </div>

      {loading ? (
        <div className="flex justify-center py-8">
          <div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
        </div>
      ) : error ? (
        <div className="text-center py-8">
          <p className="text-red-500 mb-2">Error: {error}</p>
          <Button onClick={() => fetchTopics()} variant="outline">
            Retry
          </Button>
        </div>
      ) : topics.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-500">No topics found.</p>
          <Link href="/topics/add-topic">
            <Button className="mt-2">Create Your First Topic</Button>
          </Link>
        </div>
      ) : (
        <DataTable
          columns={topicColumns}
          data={topics}
          totalItems={totalItems}
          pageIndex={currentPage - 1}
          pageSize={pageSize}
          onPageChange={handlePageChange}
          onPageSizeChange={handlePageSizeChange}
        />
      )}
    </div>
  );
}
